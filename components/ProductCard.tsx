...
export const ProductCard: React.FC<{ product: Product, onAddToCart: (p: Product) => void }> = ({ product, onAddToCart }) => (
  <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col h-full group">
    <div className="relative aspect-square overflow-hidden bg-slate-100">
      ...
      <div className="absolute top-2 right-2 px-2 py-1 bg-white/90 rounded-lg text-sm font-black text-indigo-600 shadow-sm">¥{product.price}</div>
    </div>
    <div className="p-4 flex flex-col flex-grow">
      ...
      <div className="mt-auto pt-4">
        <button onClick={() => onAddToCart(product)} className="w-full bg-indigo-600 text-white py-2 rounded-xl text-sm font-bold flex items-center justify-center gap-2 hover:bg-indigo-700 active:scale-95 transition-all">
          <Icons.Cart />加入购物车
        </button>
      </div>
    </div>
