  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
      <div className="bg-white w-full max-w-md rounded-[40px] p-8 shadow-2xl animate-in zoom-in-95">
        ...
        <p className="text-sm text-slate-500 mb-8">binbinaidapao 将为您准备 {items.length} 种精选花炮</p>
        <form onSubmit={e => { e.preventDefault(); onSubmit(f); }} className="space-y-5">
          ...
          <div className="pt-4 space-y-3">
            <button type="submit" className="w-full bg-orange-600 text-white py-5 rounded-2xl font-black text-lg shadow-xl shadow-orange-100">提交预订 (¥{total})</button>
            <button type="button" onClick={onClose} className="w-full text-slate-400 font-bold py-2">返回修改</button>
          </div>
        </form>
      </div>
    </div>
